namespace SDK.Lib
{
    ...
    public class AuxSceneEntityUserData : AuxUserData
    {
                    ...
                    if (null != bUserData)
                    {
                        ...
                        if (null != bBeingEntity)
                        {
                            if (!Ctx.mInstance.mFrameCollideMgr.isOrAddCollidedInCurFrame(aBeingEntity.getEntityUniqueId(), bBeingEntity.getEntityUniqueId()))
                            {
                                if ((aBeingEntity as BeingEntity).canInterActive(bBeingEntity))
                                {
                                    aBeingEntity.overlapToEnter2D(bBeingEntity, collisionInfo);
                                }
                            }
                        }
                    }
        ...
        void OnCollisionStay2D(UnityEngine.Collision2D collisionInfo)
        {
            AuxSceneEntityUserData aUserData = this.GetComponent<AuxSceneEntityUserData>();
            if (null != aUserData)
            {
                    ...
                    if (null != bUserData)
                    {
                        ...
                        if (null != bBeingEntity)
                        {
                            if (!Ctx.mInstance.mFrameCollideMgr.isOrAddCollidedInCurFrame(aBeingEntity.getEntityUniqueId(), bBeingEntity.getEntityUniqueId()))
                            {
                                if ((aBeingEntity as BeingEntity).canInterActive(bBeingEntity))
                                {
                                    aBeingEntity.overlapToStay2D(bBeingEntity, collisionInfo);
                                }
                            }
                        }
                    }
        ...
        void OnCollisionExit2D(UnityEngine.Collision2D collisionInfo)
        {
            AuxSceneEntityUserData aUserData = this.GetComponent<AuxSceneEntityUserData>();
            if (null != aUserData)
            {
                    ...
                    if (null != bUserData)
                    {
                        ...
                        if (null != bBeingEntity)
                        {
                            if (!Ctx.mInstance.mFrameCollideMgr.isOrAddCollidedInCurFrame(aBeingEntity.getEntityUniqueId(), bBeingEntity.getEntityUniqueId()))
                            {
                                if ((aBeingEntity as BeingEntity).canInterActive(bBeingEntity))
                                {
                                    aBeingEntity.overlapToExit2D(bBeingEntity, collisionInfo);
                                }
                            }
                        }
                    }
