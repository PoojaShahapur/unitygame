...
namespace SDK.Lib
{
    ...
    public class AutoUpdateSys
    {
        ...
        public void loadOneUpdateFile(string path, FileVerInfo fileInfo)
        {
            ...
            {
                UtilApi.delFile(Path.Combine(Ctx.m_instance.m_fileSys.getLocalWriteDir(), UtilLogic.combineVerPath(path, Ctx.m_instance.m_versionSys.m_localVer.m_path2HashDic[path].m_fileMd5)));     // 删除当前目录下已经有的 old 文件
            }
