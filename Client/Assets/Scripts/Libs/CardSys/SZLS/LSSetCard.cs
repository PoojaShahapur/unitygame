...
namespace SDK.Lib
{
    ...
    public class SetCard : InterActiveEntity
    {
        ...
        public CardItemBase info;
        void setinfo(CardItemBase i)//只有1次
        {
            //info = i;

            //transform.FindChild("pic").transform.FindChild("pic").renderer.material = info.image;
            //transform.FindChild("cost").GetComponent<Text>().text = info.cost;

            //if (info.cnname == "")
            //{
            //    transform.FindChild("name").GetComponent<Text>().text = info.name;
            //}
            //else
            //{
            //    transform.FindChild("name").GetComponent<Text>().text = info.cnname;
            //}
            //name = i.cardid;
        }
