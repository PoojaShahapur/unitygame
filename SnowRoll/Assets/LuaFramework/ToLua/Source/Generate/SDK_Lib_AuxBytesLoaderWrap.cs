...
public class SDK_Lib_AuxBytesLoaderWrap
{
	...
	static int syncLoad(IntPtr L)
	{
		try
		{
			...
			if (count == 3 && TypeChecker.CheckTypes(L, 1, typeof(SDK.Lib.AuxBytesLoader), typeof(string), typeof(SDK.Lib.MAction<SDK.Lib.IDispatchObject>)))
			{
			...
			else if (count == 4 && TypeChecker.CheckTypes(L, 1, typeof(SDK.Lib.AuxBytesLoader), typeof(string), typeof(LuaInterface.LuaTable), typeof(LuaInterface.LuaFunction)))
			{
				SDK.Lib.AuxBytesLoader obj = (SDK.Lib.AuxBytesLoader)ToLua.ToObject(L, 1);
				string arg0 = ToLua.ToString(L, 2);
