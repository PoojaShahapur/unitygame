...
function M:setBgMusic()
    ...
    self.Game_Music = GlobalNS.UtilApi.getComByPath(MusicModel, "Game_Music", "Toggle");
    if GlobalNS.CSSystem.Ctx.mInstance.mSystemSetting:hasKey("GameMusic") then
        if GlobalNS.CSSystem.Ctx.mInstance.mSystemSetting:getInt("GameMusic") == 1 then
            self.Game_Music.isOn = true;
        else
            self.Game_Music.isOn = false;
        end
    else
        self.Game_Music.isOn = true;
    end
...
function M:setLeftModel()
    local BG = GlobalNS.UtilApi.TransFindChildByPObjAndPath(self.mGuiWin, "BG");
    local Bg_Center = GlobalNS.UtilApi.TransFindChildByPObjAndPath(BG, "Bg_Center");
    local Op_Bg = GlobalNS.UtilApi.TransFindChildByPObjAndPath(Bg_Center, "Op_Bg");

    local Opt_Switch = GlobalNS.UtilApi.TransFindChildByPObjAndPath(Op_Bg, "Left_Switch");
    local OpModel = GlobalNS.UtilApi.TransFindChildByPObjAndPath(Opt_Switch, "OptModel");
    self.right = GlobalNS.UtilApi.getComByPath(OpModel, "right", "Toggle");
    self.left = GlobalNS.UtilApi.getComByPath(OpModel, "left", "Toggle");

    if GlobalNS.CSSystem.Ctx.mInstance.mSystemSetting:hasKey("LeftHanded") then
        if GlobalNS.CSSystem.Ctx.mInstance.mSystemSetting:getInt("LeftHanded") == 1 then
            self.left.isOn = true;
            self.right.isOn = false;
        else
            self.left.isOn = false;
            self.right.isOn = true;
        end
    else
        self.right.isOn = true;
        self.left.isOn = false;
...
function M:onCloseBtnClk()
    if self.BG_Music.isOn then
        ...
        GlobalNS.CSSystem.Ctx.mInstance.mSoundMgr:play(0, "Sound/Music/Login.wav", 1.0, 1, true, true);
    else
        GlobalNS.CSSystem.Ctx.mInstance.mSystemSetting:setInt("MusicModel", 0);
        GlobalNS.CSSystem.Ctx.mInstance.mSoundMgr:stop(0, "Sound/Music/Login.wav");
    end

    if self.Game_Music.isOn then
        GlobalNS.CSSystem.Ctx.mInstance.mSystemSetting:setInt("GameMusic", 1);
    else
        GlobalNS.CSSystem.Ctx.mInstance.mSystemSetting:setInt("GameMusic", 0);
    end
    ...
    if self.left.isOn then
        GlobalNS.CSSystem.Ctx.mInstance.mSystemSetting:setInt("LeftHanded", 1);
    else
        GlobalNS.CSSystem.Ctx.mInstance.mSystemSetting:setInt("LeftHanded", 2);
    end
