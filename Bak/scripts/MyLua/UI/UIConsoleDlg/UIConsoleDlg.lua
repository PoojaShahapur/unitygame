...
function M:onReady()
    ...
    self.ServerToggle.isOn = true;

    if self.locklogtoggle.isOn then
        GCtx.mLogSys:log("locked  ", GlobalNS.LogTypeId.eLogCommon);
    end
	
	local tmpLog = GlobalNS.CSSystem.Ctx.mInstance.mShareData:getTmpLog();
	self:onSetLogText(tmpLog);
end
...
function M:sendServerCmd(str)
    ...
    if nil == index then
        ...
        local cmd = string.sub(str, index+1);
        GlobalNS.CSSystem.Ctx.mInstance.mShareData:sendGMcmd(cmd);
        self:onSetLogText("<color=#00FF00>[Server指令]</color> " .. cmd);
    end
end
