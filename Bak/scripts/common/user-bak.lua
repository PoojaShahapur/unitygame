...
-- 每局比赛结束后加游戏币和糖果
function User:add_money_after_game(money, sugar, need_add_champion, destroynum,killnum,score,highest_combo, unlock_pur, roommode, rank)
    ...
    if roommode == 0 then
        ...
        if newlevel == 0 then
            ...
            self.level = newlevel
            self.log:info("本局结束,更新段位,旧段位%d,新段位%d"
                , self.level, newlevel)
             -- 更新段位排行榜数据
