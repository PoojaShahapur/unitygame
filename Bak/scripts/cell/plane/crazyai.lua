...
local log = require("log"):new("plane.crazyai")
...
local stage_search_range = {225, 1600, math.maxinteger }
local stage_cd_num = {3, 3, 3}
...
--
-- index 代表是第几个ai玩家
function CrazyAIPlayer:new(room, name)
...
function CrazyAIPlayer:change_state(state, player)
...
--闲逛结束,开始干人
function CrazyAIPlayer:find_nearest_enemy()
    ...
    for k,v in pairs(self.in_room.players) do
        if v.game_clt_id ~= 0 and v:is_dead() ~= true then
            ...
            if dis < min_dis and dis < stage_search_range[self.current_stage] then
                min_dis = dis
                dest_player = v
            end
        end
...
function CrazyAIPlayer:set_stage(stage)
    self.current_stage = stage
    -- 进入第二阶段,搜索范围和子弹发射速度重新设置
    self.timer_queue:erase(self.shot_timer_id)
    self.shot_timer_id = self.timer_queue:insert_repeat_from_now(0,  stage_cd_num[self.current_stage] * self.shot_cd * 1000, function()
        self:shoot()
        end)        
end

function CrazyAIPlayer:after_new()
	...
	log:debug("New crazy AIPlayer. account=%s,id=%d, fid=%d,speed=%f,cd=%f"
        , self.nickname, self.id,self.formation_manager:get_formation_id(), self.move_speed, self.shot_cd)
end

function CrazyAIPlayer:on_kill_other()
    self.destroynum = self.destroynum + 1
end

function CrazyAIPlayer:rpc_request(service_name, method_name, request_str, callback)
    return
end  -- rpc_request()

function CrazyAIPlayer:can_add_plane()
...
function CrazyAIPlayer:add_small_plane(is_born_add)
	 -- 增加小飞机
...
-- 通知玩家被 xxx 干掉了
function CrazyAIPlayer:notify_death(nickname, is_out_of_bound)
    for i = 1, config.player_born_plane_count do
        self:add_small_plane(true)
...
function CrazyAIPlayer:update_speed()
...
function CrazyAIPlayer:get_small_plane_by_id(planeid)
    local ret = nil
...
function CrazyAIPlayer:change_direction_on_edge()
...
function CrazyAIPlayer:change_direction_on_timer()
    if self:is_dead() == true then
        return
    end
    if self.state == STATE_ATTACK then
...
function CrazyAIPlayer:change_dir_when_attack()
    if self:is_dead() == true then
        return
    end
    --[[
...
function CrazyAIPlayer:is_close_to_edge(x, y)
...
function CrazyAIPlayer:frame_move(delta_time)
...
-- todo, ai 玩家出界后通知客户端
function CrazyAIPlayer:delete_small_plane(plane_id)
...
function CrazyAIPlayer:is_dead()
    return self.small_plane_count == 0
end

function CrazyAIPlayer:on_room_destroy()
    self.timer_queue:erase_all()
end

return CrazyAIPlayer
