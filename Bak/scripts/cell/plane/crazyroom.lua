...
local log = require("log"):new("plane.crazyroom")
local pb = require("protobuf")
local rr = require("remote_run.remote_runner")
local Player = require("plane.player")
local CrazyAIPlayer = require("plane.crazyai")
...
function CrazyRoom:get_cd_by_plane_num(plane_num)
    return 0.5
end
-- room同步所有玩家移动
function CrazyRoom:sync_pos(interval)
...
-- 创建房间
function CrazyRoom:new(roomid, destroy_handler)
    cur_room_id = roomid
    log:debug("Create room:"..cur_room_id)
    local crazy_last_millseconds = 180 * 1000
    -- 一定确保只有一个定时器挂着
    ...
    local room = {
        ...
        room_id = cur_room_id,
		-- room销毁时间
		destroytime = crazy_last_millseconds,
        createtime = c_util.get_sys_ms(),
        end_timestamp = os.time()*1000 + crazy_last_millseconds,
    ...
    room.timer_queue:insert_single_from_now(
        crazy_last_millseconds - 30 * 1000, function () room:set_ai_stage(3) end)
        --进入第2阶段,设置新的搜索范围,和发射CD
    room.timer_queue:insert_single_from_now(
        crazy_last_millseconds - 60 * 1000, function () room:set_ai_stage(2) end)
    ...
    for i = 1, config.room.hold_num do
        local aiplayer = CrazyAIPlayer:new(room, random_names_rs[all_name_index_tbl[i]].name)
    end
...
function CrazyRoom:set_ai_stage(stage)
...
function CrazyRoom:set_player_shotcd()
    local cd_msg = {
        shotcd = math_floor(self:get_cd_by_plane_num(3) * 1000),
    }
    self.only_human_player:rpc_request("plane.PlanePush", "SetShotCD", pb.encode("plane.ShotCDMsg", cd_msg))
end

function CrazyRoom:destroytime_over()
    log:debug('time is over,room Id:'..self.room_id)
    self.rankmgr:send_result_data()
    -- 更新下炼狱排行榜,pur为炼狱简称
    local today = os.date("%y%m%d")
    local player = self.only_human_player
    if player.score > 0 then
	    c_redis.command("zscore", "rankpur:" .. today, player.uid, function (reply)
            local reply_type= reply.type
            if 1 == reply_type or 4 == reply_type then
                --log:error("purgatory,return error type=%d,integer=%d,str=%s", reply_type,reply.integer, reply.str)
                -- 看下是否需要更新得分,如果本次得分超过排行榜中得分,则更新
                local score = tonumber(reply.str)
                if score == nil then score = 0 end
                if score < player.score then
	                c_redis.command("zadd", "rankpur:" .. today , {player.score, player.uid}
                        , function(reply)
                        if reply.type ~= 3 then
                            log:error("炼狱模式插入排行榜失败,%d,%s,score=%d,return error type=%d,integer=%d,str=%s"
                                , player.uid, player.account, player.score, reply.type,reply.integer, reply.str)
                        end
                    end)
                    -- 往mongodb中更新最高分
                    rr.run_mfa(player.game_clt_id.base_svr_id, "user_mgr", "update_pur_score", 
                        {player.game_clt_id.base_rpc_clt_id,player.score})
                end
            else
                self.log:error("炼狱模式,更新排行榜,return error type=%d,integer=%d,str=%s", reply_type,reply.integer, reply.str)
            end
        end)
    end
...
function CrazyRoom:calc_room_frame(interval)
...
-- 填充帧击中里面,打中能源
function CrazyRoom:full_frame_hit_energy_info(tbl, index_tbl,bullet_groupid, one_hit_energy_info)
...
-- 填充帧击中里面,打中飞机
function CrazyRoom:full_frame_hit_plane_info(tbl, index_tbl, bullet_groupid, one_hit_plane_info)
    self:check_frame_hit_bullet_group_exist(tbl, index_tbl,bullet_groupid)
    table_insert(tbl[bullet_groupid].planehits, one_hit_plane_info)
end

-- 检查tbl里面,该子弹组是否存在,不存在则创建;存在则不操作
function CrazyRoom:check_frame_hit_bullet_group_exist(tbl, index_tbl, bullet_group_id)
...
-- 每帧遍历所有玩家和子弹检测
function CrazyRoom:frame_check_hit()
    -- 广播给客户端的,批量添加新能源的消息
                ...
                if central_distance < (player.wrap_circle_radius + bullet_group.radius) then
                    ...
                    for _, hitinfo in pairs(bullet_hit_plane_pairs) do
                        ...
                        if attacker:is_dead() ~= true then
                            ...
                            if attacker.game_clt_id ~= 0 then
                                --log:info('attacker=%d,%s,killnum = %d', attacker.id, attacker.nickname, attacker.killnum)
...
function CrazyRoom:generate_food_id()
    return self.gen_food_id
end  -- generate_food_id()

-- 能源飞机也用该接口创建
function CrazyRoom:new_food(x,y,foodtype, skinid, angle, playerid, broadcast)
...
function CrazyRoom:init_foods()
    for i = 1, 100 do
        self:new_food()
    end  -- for
end  -- init_foods()

function CrazyRoom:broadcast_player_enter(player)
    local req = {
		id = player.id,
...
function CrazyRoom:broadcast(mthd, req_name, req)
...
-- 返回EnterRoomResponse.
function CrazyRoom:get_enter_room_response(player_id)
    ...
    for food_id, food in pairs(self.foods) do
        if food.playerid ~= 0 then-- 说明是玩家分裂出来的能源飞机
            if batch_add_plane_msgs[food.playerid] ~= nil then
                local one_plane = {plane_id = food.id, x = food.x, y = food.y}
            ...
            local food_msg = { food_id = food_id, x = food.x, y = food.y, }
...
function CrazyRoom:get_room_left_can_in_num()
        return 0
end

function CrazyRoom:get_leftsecs()
...
function CrazyRoom:enter(game_clt_id, account, uid, nickname, skinid, bulletid)
    assert("string" == type(nickname))
    local game_clt_id_str = game_clt_id:to_string()
    local player = Player:new(game_clt_id, account, uid, nickname,skinid, bulletid, self)

    -- 无敌通知必须要在客户端有了玩家之后
    player:notify_god(1)

    self.players[game_clt_id_str] = player
    self.only_human_player = player
    self.players_count = self.players_count + 1
    self.timer_queue:insert_single_from_now(
        0, function () self:set_player_shotcd() end)
    self.rankmgr:on_player_enter(player)
    self:erase_one_ai_player()
    log:info("进入炼狱模式,玩家 %s,%d,nick=%s,分配id=%u,gamecltstr=%s,skin=%d,bullet=%d "
        , account, uid, nickname, player.id, game_clt_id_str, skinid, bulletid)
	-- 别人广播给自己
...
-- 玩家的转向
function CrazyRoom:player_turn_to(game_clt_id, msg)
...
-- 开火
function CrazyRoom:player_fire(game_clt_id)
...
function CrazyRoom:get_player(player_id)
	for k, player in pairs(self.players) do
...
-- 返回MsAndId
function CrazyRoom:get_ms_and_id(game_clt_id)
    local player = self.players[game_clt_id:to_string()]
...
function CrazyRoom:back_hall(game_clt_id)
    local game_clt_id_str = game_clt_id:to_string()
...
function CrazyRoom:erase_player(game_clt_id)
    local game_clt_id_str = game_clt_id:to_string()
    local player = self.players[game_clt_id_str]
    if not player then return end
...
function CrazyRoom:erase_one_ai_player()
    local aiplayer = nil
    for k,v in pairs(self.players) do
...
-- 增加ai玩家,只会在玩家离开的时候调用
function CrazyRoom:add_ai_player()
...
function CrazyRoom:stop_or_start_move(game_clt_id, msg)
...
function CrazyRoom:player_req_rank_data(game_clt_id)
	local game_clt_id_str = game_clt_id:to_string()
	local player = self.players[game_clt_id_str]
	if not player then 
...
function CrazyRoom:player_split(game_clt_id)
	local game_clt_id_str = game_clt_id:to_string()
	local player = self.players[game_clt_id_str]
	if not player then 
...
function CrazyRoom:small_plane_die(game_clt_id, msg)
	local game_clt_id_str = game_clt_id:to_string()
	local player = self.players[game_clt_id_str]
	if not player then 
        log:debug("fuck, player req rank data, could not find player id=" ..game_clt_id_str)
