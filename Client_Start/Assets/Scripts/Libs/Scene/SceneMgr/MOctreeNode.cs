...
namespace SDK.Lib
{
    public class MOctreeNode : MSceneNode
    {
        ...
        public void _addToRenderQueue(MCamera cam, MOctreeCamera.Visibility visibleLevel)
        {
            ...
            {
                Ctx.mInstance.mLogSys.log("_addToRenderQueue", LogTypeId.eSceneCull);
                ...
                {
                    Ctx.mInstance.mLogSys.log("_addToRenderQueue Check Visible", LogTypeId.eSceneCull);

                    Ctx.mInstance.mLogSys.log(string.Format("walkOctree Child Octree Node Min {0}", tmp.getMinimum()), LogTypeId.eSceneCull);
                    Ctx.mInstance.mLogSys.log(string.Format("walkOctree Child Octree Node Max {0}", tmp.getMaximum()), LogTypeId.eSceneCull);

                    vis = cam.isVisible(ref tmp, ref plane);
                }
                if(vis)
                {
                    Ctx.mInstance.mLogSys.log("_addToRenderQueue Show", LogTypeId.eSceneCull);

                    mo.show(cam);
                }
                //else
                //{
                //    Ctx.mInstance.mLogSys.log("_addToRenderQueue Hide", LogTypeId.eSceneCull);
                //    mo.hide(cam);
                //}
            }
            */
        ...
        public void _removeFromRenderQueue(MCamera cam)
        {
            ...
            foreach (MMovableObject mo in mObjectsByName.Values)
            {
                Ctx.mInstance.mLogSys.log("_removeFromRenderQueue", LogTypeId.eSceneCull);
                mo.hide(cam);
            }
            */
