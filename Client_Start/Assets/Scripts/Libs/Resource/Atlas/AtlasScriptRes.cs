...
namespace SDK.Lib
{
    ...
    public class AtlasScriptRes : InsResBase
    {
        ...
        public ImageItem getImage(string spriteName)
        {
            ...
            {
                Ctx.mInstance.mLogSys.log(string.Format("Atlas is {0}, picture is {0}, cannot load", this.mAtlasPath, spriteName));
                Ctx.mInstance.mLogSys.log("Out picture list of atlas");
                foreach (var key in this.mPath2Image.Keys)
                {
                    Ctx.mInstance.mLogSys.log(string.Format("Atlas is {0}, picture is {0}", this.mAtlasPath, key));
                }
                return null;
            }
        }
