...
const dbCol mail_content_define[] = {
  ...
  { "BIN2",               zDBConnPool::DB_BIN,  sizeof(Cmd::Session::SessionObject)},
  { "BIN3",               zDBConnPool::DB_BIN,  sizeof(Cmd::Session::SessionObject)},
  { NULL,0,0}           
};
...
const dbCol mail_define[] = {
  ...
  { "ITEMID2",    zDBConnPool::DB_DWORD, sizeof(DWORD) },
  { "ITEMID3",    zDBConnPool::DB_DWORD, sizeof(DWORD) },
  { "BIN",    zDBConnPool::DB_BIN,  sizeof(Cmd::Session::SessionObject)},
  { "BIN2",    zDBConnPool::DB_BIN,  sizeof(Cmd::Session::SessionObject)},
  { "BIN3",    zDBConnPool::DB_BIN,  sizeof(Cmd::Session::SessionObject)},
  { NULL,0,0}           
...
bool MailService::doMailCmd(const Cmd::t_NullCmd *cmd,const DWORD cmdLen)
{
  ...
  switch (cmd->para)
  {
	...
	{
	    if(content.item[i].object.qwThisID!=0)
		itemNum++;
	}
	if(itemNum>0 && rev->space<itemNum)
	{
        ...
        bcopy(&content.item,&gmir.item,sizeof(Cmd::Session::SessionObject[MAX_MAILITEM]));
...
bool MailService::sendMail(DWORD h,Cmd::Session::t_sendMail_SceneSession & sm)
{
  ...
  {               
    ...
    Zebra::logger->error("[邮件]%s->%s 丢失 money=%u item=%s",sm.mail.fromName,sm.mail.toName,sm.mail.sendMoney,sm.item[0].object.strName);
    return false;
  }
...
void MailService::checkDB()
{
  ...
  */
  ...
  //删除过期的邮件
  bzero(where,sizeof(where));
  snprintf(where,sizeof(where),"STATE!=%u AND DELTIME<%u AND (ACCESSORY=0 OR ITEMGOT=1 OR TYPE=%u)",MAIL_STATE_DEL,ct.sec(),MAIL_TYPE_RETURN);
  mailStateInfo st;
  st.state = MAIL_STATE_DEL;
  ...
  bzero(where,sizeof(where));
  snprintf(where,sizeof(where) - 1,"STATE!=%u AND ACCESSORY=1 AND ITEMGOT=0 AND TYPE=%u AND DELTIME<%u",MAIL_STATE_DEL,MAIL_TYPE_MAIL,ct.sec());
  ...
  //删除10天之前的
  bzero(where,sizeof(where));
  snprintf(where,sizeof(where),"DELTIME<%u",ct.sec()-864000);
...
bool MailService::turnBackMail(DWORD mailID)
{
  ...
  snprintf(mail.text,255-1,"%s 谢绝了你发送的物品",mail.fromName);
  mail.recvMoney = 0;
  mail.recvGold = 0;
  DWORD t = mail.fromID;
