...
#pragma pack()
 ...
 //暂时屏蔽       DWORD retcode = RecordService::dbConnPool->exeSelectLimit(handle,"`ACCPRIV`",priv_define,where,NULL,1,(BYTE*)(&createchar_data.accPriv));
		...
		//ret.charinfo.JobType = createchar_data.useJob; 暂时屏蔽20141013
		//ret.charinfo.face    = createchar_data.face; 暂时屏蔽20141013
...
bool RecordTask::readCharBase(const Cmd::Record::t_ReadUser_SceneRecord *rev)
{
...
#ifdef _WC_DEBUG
  BlockTime bt(1, "RecordServer读一次档案");
#endif
  bzero(where,sizeof(where));
  ...
  Zebra::logger->debug("读取档案服务器 角色名:%s, 金钱:%u ",ret->charbase.name, ret->charbase.gold);
  Zebra::logger->debug("读取档案服务器数据,压缩数据大小(size = %u)",ret->dataSize);
...
#ifdef _WC_DEBUG
  BlockTime bt(1, "RecordServer存一次档案");
#endif
  bzero(where,sizeof(where));
  snprintf(where,sizeof(where) - 1,"`CHARID` = %u AND `AVAILABLE` = 1",rev->id);
...
bool RecordTask::delCharbase(DWORD accid, DWORD userid, bool log)
{
    ...
    {
	Zebra::logger->trace("删除角色:%u, %u 失败" ,accid, userid);
	RecordService::dbConnPool->putHandle(handle);
	return false;
    }
    Zebra::logger->trace("删除角色:%u, %u 成功" ,accid, userid);
    RecordService::dbConnPool->putHandle(handle);
    return true;
}
...
bool RecordTask::newChangeZoneCharbase(Cmd::Rolechange::t_sendUserToZone* rev)
{
    ...
    {
	Zebra::logger->error("战区 创建角色插入数据库出错 %u,%u",rev->accid,rev->userid);
	return false;
    }
    snprintf(temp_record->charbase.mapName, sizeof(temp_record->charbase.mapName), "地神界·天空城");	//设置战区地图
    ...
    case PARA_SCENE_USER_WRITE:
      {
		...
		{//log ok
		    Zebra::logger->info("[转区] 发送玩家(%s,%u,%u)数据到战区:%u",rev->charbase.name, rev->accid, rev->id, sendChar->toGameZone.id);
		}
	    }
		...
		if(RecordService::getInstance().sendCmdToSuperServer(sendChar, sizeof(Cmd::Rolechange::t_sendUserToZone)+nCmdLen))
		{//log ok
		    Zebra::logger->info("[转区] 发送玩家(%s,%u,%u)数据回原区:%u",rev->charbase.name, sendChar->accid, sendChar->userid, sendChar->toGameZone.id);
		}
	    }
            return true;
          }
        ...
        {
          Zebra::logger->error("回写档案验证失败,不能回写档案：%u,%u",rev->accid,rev->id);
        }
      }
      break;
    case PARA_SCENE_USER_READ:
      {
        ...
        else
        {
          ...
          ret.dataSize = (DWORD)PARA_SCENE_USER_READ_ERROR;
          sendCmd(&ret,sizeof(t_UserInfo_SceneRecord));
          Zebra::logger->error("添加读取记录失败,不能读取档案信息：%u,%u",rev->accid,rev->id);
          return true;
        }
      }
      break;
...
#pragma pack()
  ...
  if ((DWORD)-1 == retcode
      || 0 == retcode)
  {
    Zebra::logger->debug("没有找到记录 accid:%u",accid);
    ret.num = 0;
    bzero(ret.info,sizeof(ret.info));
  }
  else
  {
    Zebra::logger->debug("找到 %u 条合格的角色记录 accid:%u",retcode,accid);
    ret.num = retcode;
