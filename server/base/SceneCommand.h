...
namespace Cmd
{
  namespace Scene
  {
    ...
    struct t_Unreg_LoginScene : t_NullCmd
    {
      DWORD dwUserID;
      DWORD dwSceneTempID;
      DWORD changeCountryScene;
      BYTE isDriveHorse;
      WORD getAchiveCount;
      BYTE retcode;
      BYTE map[MAX_NAMESIZE+1];
      t_Unreg_LoginScene()
    ...
    const BYTE PARA_GM_COMMAND_SCENEGATE = 25;
    const BYTE GM_COMMAND_CMD_FILTER = 1;   //屏蔽指令
    struct t_gmCommand_SceneGate : public t_NullCmd
      {
	  BYTE gm_cmd;
	  DWORD dwUserID;
	  DWORD para1;
	  DWORD para2;
	  t_gmCommand_SceneGate() :t_NullCmd(CMD_SCENE, PARA_GM_COMMAND_SCENEGATE)
	  {
	      gm_cmd = dwUserID = para1 = para2 = 0;
	  }
	  t_gmCommand_SceneGate(BYTE gm_cmd, DWORD dwUserID=0,DWORD para1=0,DWORD para2=0):t_NullCmd(CMD_SCENE, PARA_GM_COMMAND_SCENEGATE),gm_cmd(gm_cmd),dwUserID(dwUserID),para1(para1),para2(para2)
	  {
	  }
      };
    //////////////////////////////////////////////////////////////
