...
zDataBM<zCardB,CardBase> &cardbm=zDataBM<zCardB,CardBase>::getMe();
zDataBM<zSkillB,SkillBase> &skillbm = zDataBM<zSkillB,SkillBase>::getMe();
zDataBM<zStateB,StateBase> &statebm = zDataBM<zStateB,StateBase>::getMe();
#if 0
...
bool loadAllBM()
{
    ...
    objectbm.listAll();
    if (!cardbm.refresh((Zebra::global["datadir"] + "CardBase.tbl").c_str())) return false;
    cardbm.listAll();
    ...
    npcbm.listAll();
    if (!statebm.refresh((Zebra::global["datadir"] + "StateBase.tbl").c_str())) return false;
    statebm.listAll();
