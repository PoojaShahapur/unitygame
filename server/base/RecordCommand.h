...
namespace Cmd
{
  ...
  namespace Record
  {
    ...
    struct t_Ret_SelectInfo_GateRecord : t_NullCmd
    {
      ...
      DWORD offLineTime;
      QWORD imgErrLock[MAX_CHARINFO];
      BYTE photoState[MAX_CHARINFO];
      DWORD kickTimes[MAX_CHARINFO];
	  ...
	  bzero(imgErrLock, sizeof(imgErrLock));
	  bzero(photoState, sizeof(photoState));
	  bzero(kickTimes, sizeof(kickTimes));
        }
    };
    ...
    const BYTE PARA_GATE_UPDATE_ZONE_STATE = 18;
    struct t_update_ZoneState : t_NullCmd
    {
	DWORD charid;
	DWORD accid;
	BYTE state;
	t_update_ZoneState() : t_NullCmd(CMD_GATE, PARA_GATE_UPDATE_ZONE_STATE)
	{
	    charid = 0;
	    accid = 0;
	    state = 0;
