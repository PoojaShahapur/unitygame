...
std::string & zXMLParser::dump(xmlNodePtr dumpNode,std::string & s,bool head)
{
  ...
  outbuf = (xmlOutputBufferPtr)xmlMalloc(sizeof(xmlOutputBufferPtr));
  if(outbuf != NULL)
  {
      memset(outbuf, 0, (size_t)sizeof(xmlOutputBufferPtr));
      outbuf->buffer = out;
      xmlNodeDumpOutput(outbuf, doc, dumpNode, 1, 1, "UTF-8");
  }
...
xmlAttrPtr zXMLParser::getNodeAttr(const xmlNodePtr node, const char* attrname)
{
    if(!node) return NULL;
    if(attrname)
    {
...
xmlAttrPtr zXMLParser::getNodeNextAttr(const xmlNodePtr node, xmlAttrPtr attrPtr)
{
    if(!node || !attrPtr) return NULL;
    return attrPtr->next;
}
