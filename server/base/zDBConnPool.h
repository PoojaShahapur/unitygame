...
*  {
*
*    {"COUNT(*)",DB_unsigned int,sizeof(unsigned int)},
*
*    {NULL,0,0}
*
*  };
...
public:
	...
	*/
	...
	/*virtual unsigned int  exeSelect(connHandleID handleID,DBRecordSet* result,DBFieldSet* table,DBRecord* column,
	DBRecord* where,DBRecord* order = NULL,
	unsigned int limit=0,
	DBRecord* groupby = NULL,DBRecord* having = NULL) = 0;*/
	...
	* 显然如果本次插入没有AUTO_INCREMENT column,返回值大于等于0是没有意义的
	*/
	virtual unsigned int exeInsert(connHandleID handleID,DBFieldSet* table,DBRecord* rec) = 0;
	...
	* \return 返回受影响的记录数,返回-1表示有错误发生
	*/
	virtual unsigned int exeDelete(connHandleID handleID,DBFieldSet* table,DBRecord* where) = 0;
	...
	* \return 返回受影响的记录数,返回-1表示有错误发生
	*/
	virtual unsigned int exeUpdate(connHandleID handleID,DBFieldSet* table,DBRecord* data,DBRecord* where) = 0;
