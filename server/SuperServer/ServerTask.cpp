	...
	case PARA_CHARNAME_GATEWAY:
		{
			...
			Zebra::logger->debug("角色名称指令：%u,%u,%s,%u",ptCmd->accid,ptCmd->charid,ptCmd->name,ptCmd->state);
			if(!RoleregClientManager::getInstance().broadcastOne(&cmd, sizeof(cmd)))
			{
			    if(ptCmd->state & ROLEREG_STATE_WRITE)
			    {
					...
					t_Charname_Gateway tCmd;
					bcopy(ptCmd,&tCmd,sizeof(tCmd));
					tCmd.state |= ROLEREG_STATE_WRITE;  //设置不成功
					sendCmd(&tCmd,sizeof(tCmd));
			    }
			}
			return true;
	...
	case Cmd::Super::CMD_SESSION:
		{
			...
			case Cmd::Super::PARA_USER_ONLINE_BROADCAST:
				...
				Zebra::logger->debug("[广播消息]准备发往广播服务器 区:%u", send->srcZone.zone);
				return true;
			    }
			    break;
		    }
...
bool ServerTask::msgParse_Battle(const Cmd::t_NullCmd *pNullCmd,const DWORD nCmdLen)
{
	...
	switch(pNullCmd->para)
	{
	    ...
	    case PARA_CHECK_VALID:
		{
		    ...
		    if(RolechangeClientManager::getInstance().broadcastOne(rev, nCmdLen))
		    {//log ok
			Zebra::logger->debug("[跨区] 向 roleChangeServer 发送 from=%u to=%u accid=%u 成功",rev->fromGameZone.zone, rev->toGameZone.zone, rev->accid);
		    }
		    else
		    {//log fail
			Zebra::logger->error("[跨区] 向 roleChangeServer 发送 from=%u to=%u accid=%u 失败",rev->fromGameZone.zone, rev->toGameZone.zone, rev->accid);
		    }
		    return true;
		    
		}
		break;
	    case PARA_RET_CHECK_VALID:	//收到档案服务发来的验证返回消息,转发给转区服务器
		{
		    t_Ret_Check_Valid* rev = (t_Ret_Check_Valid*)pNullCmd;
		    rev->fromGameZone = SuperService::getInstance().getZoneID();
		    if(RolechangeClientManager::getInstance().broadcastOne(rev, nCmdLen))
		    {//log ok
			Zebra::logger->debug("[跨区] 收到本区 RECORDSERVER 发来的验证返回消息 ,转发给roleChangeServer时成功");
		    }
		    else
		    {//log fail
			Zebra::logger->debug("[跨区] 收到本区 RECORDSERVER 发来的验证返回消息 ,转发给roleChangeServer时失败");
		    }
		    return true;
		}
		break;
...
bool ServerTask::msgParse_Scene(const Cmd::t_NullCmd *pNullCmd,const DWORD nCmdLen)
{
	...
	case PARA_FORWARD_MSG_SCENE:
		...
		Zebra::logger->debug("[跨区] 发送 from=%u to=%u size=%u",send->fromGameZone.id, send->toGameZone.id, send->size);
		return true;
	    }
	    break;
    }
