 ...
 * \param port 服务器端口
 */
FLClient::FLClient(const std::string &ip, const WORD port) : zTCPClientTask(ip,port,true),tempid(++tempidAllocator),netType(NetType_near)
{
...
int FLClient::checkRebound()
{
  ...
  if (retcode > 0)
  {
    ...
    else
    {
      ...
      {
        Zebra::logger->debug("FLServer return the gameZone info :zoneid=%u(gameid=%u,zone=%u),name=%s,nettype=%u",
            ptCmd->gameZone.id,
            ptCmd->gameZone.game,
            ptCmd->gameZone.zone,
            ptCmd->name,
      ...
      {
        Zebra::logger->error("FLServer return the gameZone info, para ERROR!!!");
        return -1;
      }
    }
  }
