...
bool LoginSessionManager::verify(const DWORD loginTempID,const DWORD accid,char *numPassword,DES_cblock *key)
{
      ...
      && loginTempID == it->second.session.loginTempID)
  {
    ...
    if (0!=key)
    {
      ...
      Zebra::logger->debug("复制DES密钥：%u %u %u %u %u %u %u %u",it->second.session.des_key[0],it->second.session.des_key[1],it->second.session.des_key[2],it->second.session.des_key[3],it->second.session.des_key[4],it->second.session.des_key[5],it->second.session.des_key[6],it->second.session.des_key[7]);
    }
