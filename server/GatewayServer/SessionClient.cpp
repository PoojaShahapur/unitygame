			...
			case PARA_GATE_UNREGUSER:
				{
					...
					if (pUser)
					{
						if (rev->retcode==UNREGUSER_RET_LOGOUT)
						{
							...
							{
								//pUser->final();
								Zebra::logger->info("Session请求用户(%u,%u,%s)注销",pUser->accid,pUser->id,pUser->name);
								pUser->TerminateWait();
								//SAFE_DELETE(pUser);
							}
							return true;
					...
					{
						Zebra::logger->warn("注销时无此用户(%u),scenetempid=%u,注销类型%d",
			...
			case PARA_FORWARD_USER:
				{
					...
					{
						//Zebra::logger->debug("转发会话服务器帐号%u的%u %u消息失败",rev->dwID,((Cmd::stNullUserCmd *)rev->data)->byCmd,((Cmd::stNullUserCmd *)rev->data)->byParam);
					}
					else
					{
						Zebra::logger->info("会话发用户%u的[cmd=%u para=%u]消息成功",rev->dwID,((Cmd::stNullUserCmd *)rev->data)->byCmd,((Cmd::stNullUserCmd *)rev->data)->byParam);
					}
					return true;
				}
				break;
