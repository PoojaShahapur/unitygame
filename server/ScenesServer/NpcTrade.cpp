...
bool NpcTrade::init()
{
  ...
  zXMLParser xml;
  if (!xml.initFile(Zebra::global["configdir"] + "npctrade.xml"))
  {
  ...
  Zebra::logger->info("加载npc交易配置文件失败");
  return false;
}
...
DWORD NpcTrade::getNpcAction(const DWORD npcid)
{
  DWORD action = 0;
  if(0 == npcid)
      return 0;
  for(NpcDialogMultiMap::const_iterator it = dialogs.begin(); it != dialogs.end(); it++)
  {
    if (it->second.npcid == npcid)
    {
	for(NpcItemMultiMap::const_iterator it2 = it->second.items.begin(); it2 != it->second.items.end(); ++it2)
	{
	    action |= it2->second.action;
	}
    }
  }
