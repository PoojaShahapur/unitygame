...
bool ScenesService::init()
{
	...
	Zebra::logger->info("服务器版本号:%d", verify_version);
	...
	{
	    Zebra::logger->info("初始化SceneRecycle模块成功");
	}
	...
	{
	    Zebra::logger->error("初始化DynamicSceneManager配置文件失败");
	    return false;
	}
	...
	if(Zebra::global["battleZone"] != "")
	{
	    ...
	    Zebra::logger->info("加载战区ID :%d", battleZoneID);
	}
...
int main(int argc,char *argv[])
{
	...
	Zebra::global["vardir"] = "game_var/";	    //存储任务相关var数据
