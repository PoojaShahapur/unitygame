...
int get_time()
{
  return time(NULL)-zRTime::my_timezone;
}
...
void updateMsgArray(int cmd, int para, int size, unsigned long long time/*毫秒时间戳*/, char *name)
{
    ...
    recentmsg[index].cmd = cmd;
    recentmsg[index].para = para;
    recentmsg[index].size = size;
    recentmsg[index].time = time;
    strncpy(recentmsg[index].name, name, MAX_NAMESIZE-1);
    index++;
