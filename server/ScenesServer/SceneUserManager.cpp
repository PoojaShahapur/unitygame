...
void SceneUserManager::removeUserByTask(SceneTask *task)
{
  ...
  for(std::vector<DWORD>::iterator iter = exec.del_vec.begin() ; iter != exec.del_vec.end() ; iter ++)
  {
    ...
    if (pUser)
    {
      ...
      if(ScenesService::getInstance().battle)	//战区下线
      {
      ...
      //pUser->killAllPets();
      pUser->unreg();
      Zebra::logger->info("用户%s(%u)因卸载网关注销",pUser->name,pUser->id);
    }
    else
    {
      ...
      if (pUser)
      {
        Zebra::logger->info("用户%s(%u)因卸载网关注销,但这个人正在读取档案",pUser->name,pUser->id);
...
void SceneUserManager::removeAllUser()
{
  ...
  for(std::vector<DWORD>::iterator iter = exec.del_vec.begin() ; iter != exec.del_vec.end() ; iter ++)
  {
    ...
    if (pUser)
    {
      ...
      Zebra::logger->info("用户%s(%u)因服务器关闭卸载",pUser->name,pUser->id);
#if 0
      OnQuit event(1);
      EventTable::instance().execute(*pUser,event);
      execute_script_event(pUser,"quit");
...
bool SceneRecycleUserManager::canRegByACCID(DWORD id)
{
    struct RecycleUserExec : public execEntry<SceneUser>
    {
	bool result;
	DWORD _accid;
	RecycleUserExec(DWORD accid)
	{
	    _accid = accid;
	    result = true;
	}
