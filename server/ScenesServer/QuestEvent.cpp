...
bool Event::find(DWORD questID)
{
    quest_iterator iter = _quests.begin(),
		   end = _quests.end();
    if(end != _quests.find(questID))
	return true;
    return false;
}
/**     
...
{  
  ...
  if (root)
  {
    ...
    bzero(_npc_name,2*MAX_NAMESIZE);
    xml.getNodePropStr(root,"name",_npc_name,2*MAX_NAMESIZE);
        ...
        while (embranchment_node) {
          if (0 == strcmp((char *)embranchment_node->name,"embranchment")  ) {  
            ...
            while (c_a_node) {
              ...
              if (0 == strcmp((char *)c_a_node->name,"orconditions")  ) {  
                
                xmlNodePtr or_conditions_node = xml.getChildNode(c_a_node,NULL);
                while (or_conditions_node) {
                  
                  MakeFunction::Maker<Condition>* maker = ConditionFactory::instance().get_creator((char *)or_conditions_node->name);
                  if (maker) {
                    Condition* condition = maker->make(xml, or_conditions_node);
                    embranchment->addOr(condition);
                  }

                  or_conditions_node = xml.getNextNode(or_conditions_node,NULL);
                }
              } //endif or_conditions
...
int Event::execute()
{
	...
	if(!it->second)
	{
	    Zebra::logger->trace("脚本异常 任务:%u", quest_id);
	    continue;
	}
	Vars vars(quest_id);
	embranchment_iterator e_it = it->second->begin();
...
int Event::execute(SceneUser& user)
{
  ...
  for (quest_iterator it=_quests.begin(); it!=_quests.end(); ++it) {
    ...
    if (vars ) {
	  ...
	  vars = user.quest_list.vars(quest_id);
          if (vars && vars->update()) vars->update(0);
          //break;
        }        
...
bool Event::Embranchment::is_valid(SceneUser* user,const Vars* vars)
{
    ...
    if(!_or_conditions.empty())
    {
	for (std::vector<Condition* >::const_iterator it=_or_conditions.begin(); it!=_or_conditions.end(); ++it) {
	    if ((*it)->is_valid(user,vars)) {
		return true;
	    }    
	}
 ...
 * \return 无
 */
void Event::Embranchment::addOr(Condition* condition)
{
  if (condition) _or_conditions.push_back(condition);
}
...
void EventManager<OnVisit>::get_valid_quest_str(SceneUser &user)
{
  ...
  if (have)
    strncpy(cmd->content, menu.str().c_str(), sizeof(buf)-(DWORD)((DWORD)(&(cmd->content))-(DWORD)cmd));
  else
    strncpy(cmd->content, "现在没有新的任务给你", sizeof(buf)-(DWORD)((DWORD)(&(cmd->content))-(DWORD)cmd));
