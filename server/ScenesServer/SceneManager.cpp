			...
			if (ScenesService::getInstance().getServerID() == id) {
				...
				while(countryNode)
				{
					...
					while(mapNode)
					{
						//TODO 判断是否可以加载这个地图
						//加载地图
						//sky 现在id已经不是单纯的数字拉,所以先以字符串的形式读出来
						char s[MAX_NAMESIZE];
						...
						//sky 检测配置字符串是否是一个动态配置文本
						Scene *loaded = NULL;

						//sky 如果不是动态配置文本就直接生成一个地图
						if(!strstr(s, ":"))
						{
							WORD mapid = atoi(s);
						...
						if (loaded)
						{
							...
							Zebra::logger->info("加载%s(%d,%d)成功",loaded->name,loaded->id,loaded->tempid);
					...
					serverCountryID = countryid;		//scenesinfo.xml加载的时候记录下serverCountryID
				}
				Zebra::logger->info("ScenesServer id=%d加载%d张地图. countryID=%u",id,mapCount,serverCountryID);

				//sky 根据战场服务器约定 id 大于BATTLEFIELD_SCENE_ID将被认定为战场服务器
				if(ScenesService::getInstance().getServerID() > BATTLEFIELD_SCENE_ID)
				{
					//sky 通知session将自己划为可分配战场地图
					t_Scene_MeIsBattlefield cmd;
					sessionClient->sendCmd(&cmd, sizeof(t_Scene_MeIsBattlefield));
					printf("ScenesServer %d 为战场服务器,并已上报session层\n", ScenesService::getInstance().getServerID());
				}
...
Scene *SceneManager::loadChallengeGameMap(WORD mapID, char* mapName, Scene *t)
{
    Scene *o = SceneManager::getInstance().getSceneByName(mapName);
    if(o)
    {
	Zebra::logger->error("[卡牌对战] %s 地图名重复", mapName);
	return NULL;
    }
    Scene *s = new StaticScene();
    if(s)
    {
	bool ret = s->initByTemplet(mapID, mapName, t);
	if(ret)
	{
	    rwlock.wrlock();
	    ret=addEntry(s);
	    rwlock.unlock();
	    if(!ret)
		Zebra::logger->error("[卡牌对战]添加 %s %u %u失败",s->name, s->id, s->tempid);
	}
	else
	{
	    Zebra::logger->error("[卡牌对战]初始化 %s %u %u失败",s->name, s->id, s->tempid);
	}
