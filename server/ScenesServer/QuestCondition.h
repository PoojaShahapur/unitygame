#ifndef _QUESTCONDITION_H_
#define _QUESTCONDITION_H_
...
template <typename Operation>
class CountrysVarCondition : public GlobalCondition<Operation>
{
public:
  CountrysVarCondition(ScenesParser& p) : GlobalCondition<Operation>(p)
  { 
  }

  virtual ~CountrysVarCondition()
  { }
  ...
  bool check_valid (const SceneUser* user,const Vars* vars) const
  {
    ...
    Vars* v = CountrysVar::instance().vars(user->charbase.country);
    if (v) {
 ...
 */
template <typename Operation>
class HeroLevelCondition : public TeamCondition
{
public:  
  typedef typename Operation::value_type value_type;
   ...
   * \param p: 关键词列表
   */       
  HeroLevelCondition(ScenesParser& p) : TeamCondition(p)
  { 
    p.value("value",_condition);  
    p.value("occupation",_occupation);  
  }
  ...
  {
      Operation op;
      if(user->ahData.inUseOccupation == _occupation)
	  return op(HeroInfoManager::getMe().getOneHeroLevel(const_cast<SceneUser *>(user), _occupation), _condition);
      else 
	  return false;
  }
