...
{
  int len = 0;
  memcpy(dest+len,&_timeout,sizeof(_timeout));
  len += sizeof(_timeout);
  memcpy(dest+len,&_start_time,sizeof(_start_time));
...
{
  int len = 0;
  memcpy(&_timeout,(dest+len),sizeof(int));
  len += sizeof(int);
  memcpy(&_start_time,(dest+len),sizeof(int));
  len += sizeof(int);
...
int Vars::save(BYTE* dest) const
{
  ...
  for (const_var_iterator it=_vars.begin(); it!=_vars.end(); ++it) {
    if (!it->second.is_tmp()) {
      ...
      memcpy(dest+len,&tmp,sizeof(int));
      len += sizeof(int);
      memcpy(dest+len,it->first.c_str(),it->first.length());
      len += it->first.length();
      ...
      Zebra::logger->debug("Vars::save 存储变量(%s:%s)",it->first.c_str(),it->second.value().c_str());
    }
  }
  //store count
  memcpy(dest + length,&count,sizeof(int));  

  return len;
}

int Vars::save(std::vector<unsigned char> &buf, int &len) const
{
    memcpy(&buf[len], &_timeout, sizeof(_timeout));
    len += sizeof(_timeout);
    memcpy(&buf[len], &_start_time,sizeof(_start_time));
    len += sizeof(_start_time);
	...
	if (!it->second.is_tmp()) {
	    ++count;
	    int tmp = it->first.length();
	    memcpy(&buf[len], &tmp, sizeof(int));
	    len += sizeof(int);
	    memcpy(&buf[len], it->first.c_str(), it->first.length());
	    len += it->first.length();

	    tmp = it->second.value().length();
	    memcpy(&buf[len], &tmp, sizeof(int));
	    len += sizeof(int);
	    memcpy(&buf[len], it->second.value().c_str(), it->second.value().length());
	    len += it->second.value().length();      
    
	    if(buf.capacity() < (DWORD)(len +1024))
	    {
		buf.resize(len+1024*5);
		Zebra::logger->warn(" 存储变量时发现变量过多，扩展内存");
	    }
	}
    }
    //store count
    memcpy(&buf[length],&count,sizeof(int));  
    return 0;
}
/**     
 * \brief 读取变量
...
int Vars::notify(SceneUser& user) const
{
  ...
  for (const_var_iterator it=_vars.begin(); it!=_vars.end(); ++it) {
    ...
    //name
    strncpy((char*)(vars->buffer+offset), it->first.c_str(), MAX_NAMESIZE);
    offset += MAX_NAMESIZE;
    DWORD size = it->second.value().length();
    size = (size>256)?256:size;
    *((WORD*)(vars->buffer +offset)) = size;
    offset += sizeof(WORD);
    //value
    strncpy((char*)(vars->buffer+offset),it->second.value().c_str(), size);
    offset += size;
    //Zebra::logger->debug("var:(%s,%s)",it->first.c_str(),it->second.value().c_str());
  }
  vars->count = offset;
  if(vars->count)
      user.sendCmdToMe(vars,sizeof(Cmd::stQuestVarsUserCmd)+ vars->count);
...
{  
  ...
  if (it != _vars.end()) {
    BYTE buf[sizeof(Cmd::stQuestVarsUserCmd) + (MAX_NAMESIZE + 256)];
    Cmd::stQuestVarsUserCmd* vars = (Cmd::stQuestVarsUserCmd*)buf;
    constructInPlace(vars);
    vars->id  = _quest_id;
...
int UserVar::VAR::save(BYTE* dest) const
{
  ...
  for (const_var_iterator it=_vars.begin(); it!=_vars.end(); ++it) {
      ++count;
      memcpy(dest+len,&(it->first),sizeof(it->first));
      len += sizeof(it->first);
...
void QuestList::add_quest(DWORD id,const Vars& vars,SceneUser& user,bool notify)
{
  ...
  if (quest ) {
    ...
    memcpy(info->name,quest->title().c_str(),len);
    info->name[len] = '\0';
    //info->length = quest->description().length();
    info->start = start_time(id);
    //Zebra::logger->debug("quest time(%d)",info->start);
    memcpy(info->info,quest->description().c_str(),quest->description().length());
 ...
 */
bool QuestList::set_menu(const std::string& menu, const Vars* vars)
{
    DWORD id = vars->quest_id();
    const_quest_menu it = _menu.find(id);
    if(it != _menu.end())
    {
	return false;
    }
...
int QuestList::get_menu(char* menu,int& status)
{
  int len = 0;
  std::string allmenu = "";
  for(const_quest_menu it = _menu.begin(); it != _menu.end(); ++it)
  {
      allmenu += it->second;
      allmenu += "\n";
  }
  ...
  if (!allmenu.empty() ) {
    ...
    len = ((int)allmenu.length()>len)?len:allmenu.length();
    strncpy(menu, allmenu.c_str(), len);
...
int QuestList::save(BYTE* dest) const
{
  ...
  for (const_quest_iterator it=_quests.begin(); it!=_quests.end(); ++it) {
    ...
    Zebra::logger->debug("存储任务(%u)",it->first);
    len += sizeof(DWORD);
