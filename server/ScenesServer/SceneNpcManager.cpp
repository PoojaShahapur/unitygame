...
void SceneNpcManager::removeNpcInOneScene(Scene *scene)
{
	...
	if (pNpc)
	{
	    scene->removeNpc(pNpc);
	    removeSceneNpc(pNpc);
	    removeSpecialNpc(pNpc);
	    SceneRecycle::getMe().insert(pNpc);
	    pNpc = 0;
	}
    }
}
...
{
  zXMLParser xml;
  if (!xml.initFile(Zebra::global["configdir"] + "NpcCommonChat.xml"))
  {
...
{
  if (NpcCommonChatTable[type].empty() || !zMisc::selectByPercent(NpcCommonChatRate[type])) return false;

  int index = zMisc::randBetween(0,NpcCommonChatTable[type].size()-1);
