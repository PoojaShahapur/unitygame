...
class scriptTaskManagement : public Singleton<scriptTaskManagement>
{

friend class SingletonFactory<scriptTaskManagement>;
public:
...
public:
	...
	~userScriptTaskContainer()
	{
		for(int i = 0; i < (int)taskType::end; ++i)
		{
			if(tasks[i] != 0)
			{
				scriptTaskManagement::getMe().deleteTask(tasks[i]);
			}
		}
	}
	...
	bool add(taskType _type,scriptTask *_task)
	{
		...
		if(scriptTaskManagement::getMe().addTask(_task))
		{
	...
	bool del(taskType _type)
	{
		if(tasks[_type] == 0)
			return false;
		if(scriptTaskManagement::getMe().deleteTask(tasks[_type]))
		{
			tasks[_type] = 0;
			return true;
		}
