#ifndef _QUESTACTION_H_
#define _QUESTACTION_H_
#include "SceneUser.h"
 ...
 *
 */
class AddHeroExpAction : public TeamAction
{
public:
   ...
   * \param p: 关键词列表
   */       
  AddHeroExpAction(ScenesParser& p) : TeamAction(p)
  { 
...
class TraceLogAction : public Action
{
    public:
	TraceLogAction(ScenesParser& p)
	{
	    p.value("name", _name);
	    p.value("state", _state);
	}
	~TraceLogAction()
	{}
	int done (SceneUser* user,Vars* vars);
...
template <typename Operation>
class CountrysVarAction : public GlobalAction<Operation>
{
public:
  typedef typename Operation::value_type value_type;
  
  CountrysVarAction(ScenesParser& p) : GlobalAction<Operation>(p)
  { 
  }

  virtual ~CountrysVarAction()
  { }
  ...
  int done (SceneUser* user,Vars* vars)
  {
    ...
    Vars* v = CountrysVar::instance().add(user->charbase.country);
    if(v)
