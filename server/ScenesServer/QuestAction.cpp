...
{
  user->quest_list.set_menu(_menu, vars);
  return Action::SUCCESS;
}
 ...
 * \brief  增加英雄经验
 *
 * 重载了done函数,在增加当前英雄的经验
 *      
 * \param user: 触发动作的用户
 * \param vars: 用户所带的该任务相关变量
 * \return SUCCESS表示成功,FAILED表示失败,DISABLE表示禁用某项功能
 */
int AddHeroExpAction::done (SceneUser* user,Vars* vars)
{
    if(_occupation == user->ahData.inUseOccupation)
    {
	HeroInfoManager::getMe().addOneHeroExp(*user, _occupation, _value);
    }
    else
    {
...
int TraceLogAction::done (SceneUser* user,Vars* vars)
{
    DWORD id = vars->quest_id();
    Zebra::logger->trace("任务:%u 角色:(名字:%s,accid:%u,charid:%u) %s",id, user->name, user->charbase.accid, user->id, _name.c_str());
    return Action::SUCCESS;
}
#if 0
/**     
