...
{
    Zebra::logger->debug("SceneTask::verifyConn");
	int retcode = mSocket.recvToBuf_NoPoll();
	Zebra::logger->debug("SceneTask::verifyConn retcode:%d", retcode);
	if (retcode > 0)
	{
		BYTE pstrCmd[zSocket::MAX_DATASIZE];
		int nCmdLen = mSocket.recvToCmd_NoPoll(pstrCmd,sizeof(pstrCmd));
		Zebra::logger->debug("SceneTask::verifyConn retcode:%d nCmdLen:%d", retcode,nCmdLen);
		if (nCmdLen <= 0)
...
{
#if 0
    CommandTime command_time(1/*微秒*/, "usermsgParse", pNullCmd->cmd, pNullCmd->para);
#endif
	using namespace Cmd;

	updateMsgArray(pNullCmd->cmd, pNullCmd->para, cmdLen, SceneTimeTick::currentTime.msecs(), pUser->name);
	
	if(!pNullCmd || cmdLen == 0)
	    return false;
#if 0  //先屏蔽掉,这个以后的以后可以尝试使用
	MsgDelegate *msg_dg = MsgCenter::getMe().find(pNullCmd->cmd, pNullCmd->para);
	if(msg_dg && msg_dg->delegate<SceneTask, SceneUser, SceneTask_Callback>(this, pUser, pNullCmd, cmdLen))
	{
	    return true;
	}
	else
	{
	    Zebra::logger->error("[消息处理] MSG(cmd=%u, para=%u)失败", pNullCmd->cmd, pNullCmd->para);
	    return false;
	}
#endif
	...
	if (pUser->getState() == SceneUser::SceneEntry_Death)
	{
		...
		default:
			{
				//Zebra::logger->debug("(%s,%u)死亡状态过滤指令 (%u,%u)",pUser->name,pUser->id,pNullCmd->cmd,pNullCmd->para);
				return true;
			}
			break;
		}
		...
		case MAPSCREEN_USERCMD:
			{
				...
				default:
					{
						Zebra::logger->debug("(%s,%u)交易状态过滤指令 (%u,%u)",pUser->name,pUser->id,pNullCmd->cmd,pNullCmd->para);
						return true;
					}
					break;
				}
		...
		case MAPSCREEN_USERCMD:
			{
				...
				default:
					{
						Zebra::logger->debug("(%s,%u)摆摊状态过滤指令 (%u,%u)",pUser->name,pUser->id,pNullCmd->cmd,pNullCmd->para);
						return true;
					}
					break;
				}
	...
	case HERO_CARD_USERCMD:
		{
		    bool ret = pUser->doHeroCardCmd((Cmd::stHeroCardCmd *)pNullCmd, cmdLen);
		    return ret;
		}
	case CHAT_USERCMD:
	...
	if (pNullCmd->para==Cmd::Scene::PARA_LOGIN_UNREG)
	{
		...
		//if (scene)
		{
			...
			if (pUser)
			{
				...
				pUser->unreg();
				Zebra::logger->debug("用户%u注销时还没有进入场景",rev->dwUserID);
			}
			else
			{
				...
				if (pUser)
				{
					...
					Zebra::logger->error("注销时,未找到临时编号为%u的用户",rev->dwUserID);
			}
		}
		return true;
	}
...
bool SceneTask::cmdMsgParse(const Cmd::t_NullCmd *cmd,const DWORD cmdLen)
{
	...
	if (cmd->cmd==Cmd::Scene::CMD_FORWARD && cmd->para==Cmd::Scene::PARA_FORWARD_SCENE)
	{
		...
		//if (scene)
		{
			...
			{
				Zebra::logger->error("处理用户指令(%d,%d)时,未找到临时编号为%u的用户",cmd->cmd,cmd->para,rev->dwUserID);
				//        SceneUserManager::getMe().unlock();
				return true;
			}
		}
...
bool SceneTask::psstReqAllCardTujianDataUserCmd(SceneUser *pUser, const Cmd::t_NullCmd *pNullCmd, const DWORD cmdLen)
{
    Zebra::logger->debug("处理了一个psstReqAllCardTujianDataUserCmd消息");
    return true;
}
